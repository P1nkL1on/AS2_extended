class export_block {
	...
	// Сделать в него обрабочтик ошибок!
		...
		// jumpBack - в случае, если коллизия с зелмей все таки возможна, то устанавливается коэффицент отпрыгивания - он положителен и меньше 1. Пропорционально ему скорость по у меняется при ударе.
			static function export_object (where, path:String, time:Number, x0, y0, sp_x0, sp_y0, acs, tormoz, mass , ignore_ground ,  jumpBack, max_sp_y ){
				//default values
					...
					if (mass == undefined) mass = 1; if (jumpBack == undefined) jumpBack = 0; if (ignore_ground == undefined) ignore_ground = false;									// стандартные значения оставшихся параметрнов
						...
						// set vairables
								inter_block.set_moveble(s,acs, tormoz, jumpBack, mass, max_sp_y); s._x = x0; s._y = y0; s.sp_x0 = sp_x0; s.sp_y = sp_y0; s.stop();  s.ignore_ground = ignore_ground; s.timer = time;
						// move as you want
						// Объект удаляется со сцены если 1. его таймер истек, 2. он не касается камеры и при этом не подбираемый (для экономии памяти)., предметы остаются предметами даже если ты а них не смотришь. 
								s.onEnterFrame = function (){	if (this.InnerEnterFrame!= undefined)this.InnerEnterFrame();
																inter_block.being_moveble(this, ignore_ground); if (this.timer>0 && _root.timeElapsed!=undefined){this.timer -= _root.timeElapsed; if (this.timer<=0 || (! this.hitTest(_root.cam) && _root.cam != undefined && !this.isPickUp))this.removeMovieClip();} }
		...
		// В path - не надо дописывать _effect. Кстати, все пути эффектов должны заканчиваться на _effect.			
			static function export_effect (where, path:String, x0, y0, angle){
				// in case of economic an graphical memory
				if (_root.effect_disable == true){ _root.lastEffect = null; return;}
